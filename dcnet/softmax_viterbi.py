...
adc04: 0.7586
mirex05: 0.8348
mdb:  0.8180/0.7947/0.7902(training/validation/test)
mir1k: 0.6986
rwc: 0.7422
...
Mar-2023 viterbi
validation:
p   scale   oa_viterbi  oa
0.8 2       0.7990      0.7949  (chosen)
0.5 2       0.7946
0.95    2   0.7983
0.7 2       0.7987
0.8 4       0.7987
...
*** all the results below were calculated by mir-eval ***

training/validation/test oa-viterbi
0.8278/0.7990/0.8046
training/validation/test oa-original (hard threshold)
0.8249/0.7949/0.8005



viterbi decoding tested on more datasets: before/after
mdb: 0.8005 0.8046
adc04 0.7656 0.7480
mirex05 0.8371 0.8273
mir1k 0.6933 0.6785
rwc 0.7472 0.7506


8-May-2023 Mon: ablation study
p scale mir_eval oa
0.5 1   training/validation/test: hard thresholding viterbi
                                  0.8249/0.7949/0.8005            0.7855/0.7755/0.7643
0.5 2                                                             0.8220/0.7946/0.7980
0.8 1                                                             0.8247/0.7981/0.8013
0.8 2                                                             0.8278/0.7990/0.8046


19-May-2023 Fri
comparision with softmax (mir_eval)

softmax-unscaled training/validation/test 0.7899/0.7763/0.7652

softmax-scaled: 0.5411/0.5836/0.5302
...
class Config:
    ...
    def __init__(self):
        ...
        self.viterbi_method = self.allowed_viterbi_methods[1]

        self.train_or_inference = Namespace(
            inference='/media/ssd/music_trans/2209/9_is_vocals/m2m3/ckpts_d1/d1-16',
            from_ckpt=None,
            ckpt_prefix=None
        )
        self.tb_dir = 'tb_softmax_scaled'
    ...
    def initialize_optimizer_fn(self):
        ...
        spec = tf.random.normal([1, self.snippet_len, 500])
...
class AcousticModel:
    ...
    def __init__(self, config):
        ...
        self.global_kernel_idx = self.locate_global_kernel_fn()
        self.global_kernel = self.trainable_variables[self.global_kernel_idx]
        self.n_trainables = len(self.trainable_variables)
        self.wd_coeff = tf.Variable(2e-4, trainable=False, name='wd_coeff')

        viterbi_method = config.viterbi_method
        if viterbi_method == 'shaun':
            viterbi_ins = Viterbi()
        elif viterbi_method == 'softmax-scaled':
            viterbi_ins = SoftMaxViterbi(voicing_threspold_prob=self.voicing_threshold, scaled=True)
        elif viterbi_method == 'softmax-unscaled':
            viterbi_ins = SoftMaxViterbi(voicing_threspold_prob=self.voicing_threshold, scaled=False)
        else:
            assert False
        self.viterbi_ins = viterbi_ins

    def locate_global_kernel_fn(self):

        model = self.acoustic_model
        layer = model.get_layer('global/conv')
        w = layer.trainable_variables
        assert len(w) == 1
        w = w[0]
        assert w.name == 'global/conv/kernel:0'
        assert w.shape == (1, 97, 16, 128)
    ...
    def add_wd_grad_fn(self, grads):

        assert len(grads) == self.n_trainables
        global_kernel_idx = self.global_kernel_idx

        grad = grads[global_kernel_idx]
        w = self.global_kernel
        assert grad.shape == w.shape
        assert grad.dtype == w.dtype
        grad = grad + self.wd_coeff * w
        grads[global_kernel_idx] = grad
...
class SoftMaxViterbi:
    ...
    def __init__(self, voicing_threspold_prob, scaled):

        """
        :param voicing_threshold: a probability threshold
        """

        self.voicing_threshold_prob_tf_var = voicing_threspold_prob
        assert isinstance(self.voicing_threshold_prob_tf_var, tf.Variable)
        self.scaled = scaled
    ...
    def observation_probs_fn(self, logits):
        ...
        if self.scaled:
            ...
            ini_probs = np.ones([n_bins + 1], dtype=np.float32)

        assert isinstance(logits, np.ndarray)
        assert logits.dtype == np.float32
        assert logits.ndim == 2
        assert logits.shape[1] == n_bins
        vth_logits = self.voicing_threshold_prob_tf_var.numpy()
        vth_logits = np.log(vth_logits / (1. - vth_logits))
        logits = np.pad(logits, [[0, 0], [1, 0]], mode='constant', constant_values=vth_logits)
...
class MetricsInference(MetricsBase):
    ...
    def __init__(self, model):
        ...
        self.viterbi_ins = model.config.acoustic_model_ins.viterbi_ins
    ...
    def update_states(self, rec_idx, snippet_idx, ref_notes, logits, loss):
        ...
        if _snippet_idx == num_snippets - 1:
            ...
            voiced_flags, bins = self.viterbi_ins(logits)
