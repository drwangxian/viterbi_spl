...
class Config:
    ...
    def __init__(self):
        ...
        self.tvt_split_dict['training'] = ['AimeeNorwich_Child']
        self.tvt_split_dict['validation'] = ['Mozart_DiesBildnis']
        self.tvt_split_dict['test'] = ['MatthewEntwistle_Lontano']
...
class MetricsInference(MetricsBase):
    ...
    def effect_of_viterbi_fn(self, viterbi_est_voicing, viterbi_est_notes, no_viterbi_est_voicing, no_viterbi_est_notes):
        ...
        assert len(viterbi_est_voicing) == len(viterbi_est_notes) == len(no_viterbi_est_voicing) == len(no_viterbi_est_notes) == len(ref_notes)

        ref_notes = ref_notes.copy()

        if rec_name == 'MatthewEntwistle_Lontano':

            output_file_name = 'shaun_' + rec_name + '.npz'

            _viterbi_est_notes = viterbi_est_notes / 5. + TFDataset.note_min

            np.savez(
                output_file_name,
                ref_notes=ref_notes,
                viterbi_est_voicing=viterbi_est_voicing,
                viterbi_est_notes=_viterbi_est_notes,
                no_viterbi_est_voicing=no_viterbi_est_voicing,
                no_viterbi_est_notes=no_viterbi_est_notes
            )
