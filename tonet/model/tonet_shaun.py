...
class TONet(pl.LightningModule):
    ...
    Args:
        mode: ["disable", "enable"]
    """
    def __init__(self, l_model, r_model, config, loss_func, mode="single", datasets_list=None):
        super().__init__()
    ...
    def validation_step(self, batch, batch_idx, dataset_idx):
        ...
        if self.mode == "single":
            ...
            for i in range(0, len(cfps), mini_batch):
                ...
                temp_output = temp_output.detach().cpu().numpy()
                temp_output = np.argmax(temp_output, axis=1).astype(np.int32)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if self.mode == "single" or self.mode == "tcfp":
            ...
            print('*** average *** ')
            for dataset_idx, max_indices_list in enumerate(validation_step_outputs):
                dataset_ins = self.dataset_list[dataset_idx]
                num_recs = len(dataset_ins.rec_names)
                assert len(max_indices_list) == num_recs
                oas = []
                for rec_idx, max_indices in enumerate(max_indices_list):
                    num_freq_fames = dataset_ins.data_len[rec_idx]
                    assert len(max_indices) == num_freq_fames
                    pred_freqs = self.centf[max_indices]
                    times = np.arange(num_freq_fames) * 0.01
                    ref_freqs = dataset_ins.ref_freqs[rec_idx]
                    assert len(ref_freqs) == num_freq_fames
                    oa = mir_eval.melody.evaluate(ref_time=times, ref_freq=ref_freqs, est_time=times, est_freq=pred_freqs)['Overall Accuracy']
                    oas.append(oa)
                    print('dataset idx - {} rec idx - {} - oa - {}'.format(dataset_idx, rec_idx, oa))
                oa = np.mean(oas)
                print('dataset idx - {} - ave oa - {}'.format(dataset_idx, oa))
            ...
            for dataset_idx, max_indices_list in enumerate(validation_step_outputs):
                ...
                for rec_idx, max_indices in enumerate(max_indices_list):
                    ...
                    assert len(max_indices) == num_freq_fames
                    pred_freqs = self.centf[max_indices]
                    pred_freqs_list.append(pred_freqs)
                    ref_freqs = dataset_ins.ref_freqs[rec_idx]
                    ref_freqs_list.append(ref_freqs)

                pred_freqs = np.concatenate(pred_freqs_list)
                ref_freqs = np.concatenate(ref_freqs_list)
                assert len(pred_freqs) == len(ref_freqs)
                times = np.arange(len(ref_freqs)) * 0.01
                oa = mir_eval.melody.evaluate(ref_time=times, ref_freq=ref_freqs, est_time=times, est_freq=pred_freqs)['Overall Accuracy']
                print('dataset idx - {} - ave oa - {}'.format(dataset_idx, oa))
