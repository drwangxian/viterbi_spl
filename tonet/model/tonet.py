...
class TONet(pl.LightningModule):
    ...
    def tone_decoder(self, tone_feature):
        if self.mode == "all" or self.mode == "spat":
            ...
            for tone_layer in self.tone_attn:
                tone_h, tone_weight = tone_layer(tone_h, slf_attn_mask = None)  # [6, 128, 2048]
            tone_prob = self.tone_linear(tone_h)  # [6, 128, 12]
            tone_prob = tone_prob.permute(0, 2, 1).contiguous()  # [6, 12, 128]
        elif self.mode == "spl":
    ...
    def forward(self, x, tx = None):
        if self.mode == "single":
            ...
            _, output_l = self.l_model(x)  # [6, 1, 361, 128]
            _, output_r = self.r_model(tx)  # [6, 1, 361, 128]
            bm_l = output_l[:, :, 0, :].unsqueeze(dim = 2)  # [6, 1, 1, 128]
            output_l = output_l[:,:, 1:,:]  # [6, 1, 360, 128]
            bm_r = output_r[:, :, 0, :].unsqueeze(dim = 2)  # [6, 1, 1, 128]
            output_r = output_r[:,:, 1:,:]  # [6, 1, 360, 128]
            feature_agg = torch.cat((output_l, output_r), dim = 2)  # [6, 1, 720, 128]
            feature_agg = feature_agg.squeeze(dim = 1)  # [6, 720, 128]
            feature_agg_mi = self.tcfp_linear(feature_agg) # [6, 360, 128]
            bm_agg = torch.cat((bm_l, bm_r), dim = 2)  # [6, 1, 2, 128]
            bm_agg = bm_agg.squeeze(dim = 1)  # [6, 2, 128]
            bm_agg_mi = self.tcfp_bm(bm_agg)  # [6, 1, 128]
            bm_agg = bm_agg.permute(0,2,1)  # [6, 128, 2]
            tone_feature = feature_agg.permute(0,2,1).contiguous()  # [6, 128, 720]
            octave_feature = feature_agg.permute(0,2,1).contiguous()  # [6, 128, 720]
            tone_prob = self.tone_decoder(tone_feature)  # [6, 12, 128], no real probs, instead it is logits
            octave_prob = self.octave_decoder(octave_feature)  # [6, 8, 128], no real probs, instead it is logits

            tone_bm = self.tone_bm(bm_agg)  # [6, 128, 2] --> [6, 128, 1]
            octave_bm = self.octave_bm(bm_agg)  # [6, 128, 2] --> [6, 128, 1]
            tone_bm = tone_bm.permute(0,2,1)  # [6, 1, 128]
            octave_bm = octave_bm.permute(0,2,1)  # [6, 1, 128]

            tone_prob = torch.cat((tone_prob, tone_bm), dim = 1)  # [6, 13, 128]
            octave_prob = torch.cat((octave_prob, octave_bm), dim = 1)  # [6, 9, 128]
            
            final_feature = torch.cat((tone_prob, octave_prob, feature_agg_mi, bm_agg_mi), dim = 1)  # [6, 383, 128]
            final_feature = self.final_linear(final_feature)  # [6, 360, 128]
            final_feature = torch.cat((bm_agg_mi, final_feature), dim=1)  # [6, 361, 128]
            final_feature = nn.Softmax(dim = 1)(final_feature)  # [6, 361, 128], need to modify
            tone_prob = nn.Softmax(dim = 1)(tone_prob)  # [6, 13, 128], need to modify
            octave_prob = nn.Softmax(dim = 1)(octave_prob)  # [6, 9, 128], need to modify
    ...
    def training_step(self, batch, batch_idx):
        ...
        cfps = torch.tensor(cfps, device=device_type)
        tcfps = torch.tensor(tcfps, device=device_type)
        gds = torch.tensor(gds, device=device_type)  # (6, 128), float64
        ...
        if self.mode == "single":
            ...
            gd_maps = torch.zeros((cfps.shape[0], cfps.shape[-2] + 1, cfps.shape[-1])).to( device_type)  # (6, 361, 128)
            tone_maps = torch.zeros((cfps.shape[0], self.config.tone_class + 1, cfps.shape[-1])).to(device_type)  # (6, 13, 128)
            octave_maps = torch.zeros((cfps.shape[0], self.config.octave_class + 1, cfps.shape[-1])).to(device_type)  # (6, 9, 128)
            tone_index = ((gds % 60) / 60 * self.config.tone_class).long()  # (6, 128) gds --> int, gds % 60 // 5
            octave_index = (gds // 60 + 2).long()  # (6, 128), 0 -- > 2, 360 --> 8
            tone_index[gds < 1.0] = self.config.tone_class  # 0 (unvoiced) --> 12
            octave_index[gds < 1.0] = self.config.octave_class  # 0 (unvoiced) --> 8
            ...
            gd_map = torch.cat([tone_maps, octave_maps, gd_maps], dim = 1)  # [6, 383, 128]
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if self.mode == "single" or self.mode == "tcfp":
            for i, dataset_d in enumerate(validation_step_outputs):  
                ...
                self.print("\n")
                _dataset_name = ['adc', 'mdb'][i]
                print("\nDataset ", _dataset_name, " OA: ", metric[-1], '\n')
