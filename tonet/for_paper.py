...
w/ viterbi via mir_eval:

adc04: 0.7172
mirex05: 0.8189
mdb: 0.8882/0.8005/0.7798
mir1k: 0.6530
rwc: 0.7400
...
softmax: peak single-side width = 5
training/validation/test(mir_eval): 0.8407/0.7713/0.7381

when peak single-side width = 15
0.8406/0.7710/0.7379
...
class Config:
    ...
    def __init__(self):
        ...
        self.allowed_viterbi_methods = ('shaun', 'softmax')
        self.viterbi_method = self.allowed_viterbi_methods[0]
        ...
        self.tvt_split_dict['training'] = ['AimeeNorwich_Child', 'MusicDelta_Rock']
        self.tvt_split_dict['validation'] = ['Mozart_DiesBildnis', 'ClaraBerryAndWooldog_AirTraffic']
        self.tvt_split_dict['test'] = ['StrandOfOaks_Spacestation', 'MatthewEntwistle_Lontano']
...
class AcousticModel:
    ...
    def __init__(self, config):
        ...
        self.voicing_threshold = 0.32

        if config.viterbi_method == 'shaun':
            self.viterbi = Viterbi(self.voicing_threshold)
        else:
            self.viterbi = SoftMaxViterbi()
...
class SoftMaxViterbi:
    ...
    def __init__(self):
        ...
        self.h = 441
        self.sr = 44100
        self.single_side_peak_width = 15
    ...
    def observation_probs_fn(self, logits):
        ...
        assert logits.shape[1] == self.num_freq_bins + 1
        ...
        for frame_idx, are_peaks in enumerate(are_peaks_ts):
            ...
            if n_peaks == 1:
                ...
                prob_ts[frame_idx, 0] = 1
                continue

            peak_logits = logits[frame_idx, peak_indices]
            max_logit = np.max(peak_logits)
            peak_logits -= max_logit
            peak_logits = np.exp(peak_logits)
            peak_logits = peak_logits / np.sum(peak_logits)
...
class MetricsInference:
    ...
    def __init__(self, model):
        ...
        self.model = model
        self.viterbi_method = model.config.viterbi_method
    ...
    def update_states_tf_fn(self, rec_idx, ref_notes, viterbi_est_voicing, viterbi_est_bins):
        ...
        est_notes = MetricsInference.est_notes_360_fn(viterbi_est_bins)
        est_voicing = viterbi_est_voicing
        tf.debugging.assert_equal(tf.size(est_voicing), tf.size(ref_voicing))
        n_est_voicing = tf.logical_not(est_voicing)
    ...
    def no_viterbi_est_voicing_and_notes_fn(logits, padded_frames):

        logits = tf.convert_to_tensor(logits, tf.float32)
        logits.set_shape([None, 361, 128])

        padded_frames = tf.convert_to_tensor(padded_frames, tf.int32)
        padded_frames.set_shape([])

        logits = tf.transpose(logits, [0, 2, 1])
        logits.set_shape([None, 128, 361])
        logits = tf.reshape(logits, [-1, 361])
        logits = tf.cond(padded_frames > 0, lambda: logits[padded_frames:], lambda:logits)

        logits_argmax = tf.argmax(logits, axis=1, output_type=tf.int32)
        logits_argmax.set_shape([None])
        ...
        est_notes = tf.gather(note_range, logits_argmax)
        est_notes.set_shape([None])
        est_voicing = logits_argmax > 0

        return est_voicing, est_notes

    def update_states(self, rec_idx_tf, batch_idx_tf, padded_frames_tf, pitch_logits_torch):

        model = self.model

        if DEBUG:
            t = [rec_idx_tf, batch_idx_tf, padded_frames_tf]
            assert all(isinstance(v, tf.Tensor) for v in t)

            assert torch.is_tensor(pitch_logits_torch)
            ssf(pitch_logits_torch, [None, 361, 128])
        ...
        logits = pitch_logits_torch.cpu().numpy()

        # no viterbi
        no_viterbi_est_voicing, no_viterbi_est_notes = MetricsInference.no_viterbi_est_voicing_and_notes_fn(
            logits=logits,
            padded_frames=padded_frames
        )
        no_viterbi_est_voicing = no_viterbi_est_voicing.numpy()
        no_viterbi_est_notes = no_viterbi_est_notes.numpy()
        self.est_voicing.append(no_viterbi_est_voicing)
        self.est_notes.append(no_viterbi_est_notes)

        logits = np.transpose(logits, [0, 2, 1])  # (6, 128, 361)
        if self.viterbi_method == 'shaun':
            logits = logits[..., 1:] - logits[..., :1]  # (6, 128, 360)
            logits = np.reshape(logits, [-1, 360])
        else:
            logits = np.reshape(logits, [-1, 361])
        if padded_frames:
        ...
        if _batch_idx == num_batches - 1:
            ...
            viterbi_voicing, viterbi_bins = model.config.acoustic_model_ins.viterbi(logits)
            ...
            oa = self.mir_eval_oa_fn(_rec_idx, est_notes_with_voicing_info)
            self.mir_eval_oas.append(oa)

        if _batch_idx == num_batches - 1:
            self.effect_of_viterbi_fn(
                viterbi_est_voicing=viterbi_voicing,
                viterbi_est_notes=est_notes_with_voicing_info,
                no_viterbi_est_voicing=np.concatenate(self.est_voicing),
                no_viterbi_est_notes=np.concatenate(self.est_notes)
            )
    ...
    def effect_of_viterbi_fn(self, viterbi_est_voicing, viterbi_est_notes, no_viterbi_est_voicing, no_viterbi_est_notes):
        ...
        if rec_name == 'MatthewEntwistle_Lontano':

            output_file_name = rec_name + '.npz'
            np.savez(
                output_file_name,
                ref_notes=ref_notes,
...
def main():
    ...
    aug_info = []
    aug_info.append('viterbi method - {}'.format(MODEL_DICT['config'].viterbi_method))
    ...
    def validation_and_inference_fn(model_name, global_step=None):
        ...
        oa = model.metrics.oa
        logging.info(f'{model.name} - step - {global_step} - oa - {oa}')
