...
adc04: 0.6896
mirex05: 0.8052
mdb (training/validation/test): oa - 0.8815/0.7930/0.7754 (mir_eval), loss - 0.5920/1.0141/1.1319
mir1k: 0.6416
rwc: 0.7269
...
viterbi:
threshold = 0.32
validation oa (tf) = 0.7957
before viterbi, validation oa (tf) = 0.7950
"""
...
class Config:
    ...
    def __init__(self):
        ...
        self.labeling_method = self.allowed_labeling_methods[2]

        self.train_or_inference = Namespace(
            inference='/media/ssd/music_trans/2210/4_tonet_singer_and_m2m3_labelings/m2m3/ckpts/d0-17',
            from_ckpt=None,
            ckpt_prefix=None
        )
        self.tb_dir = 'tb_inf'
        ...
        self.tvt_split_dict['test'] = self.tvt_split_dict['test'][:1]
        if self.debug_mode:
...
class MetricsInferenceWithVoicingDetection:
    ...
    def mir_eval_oa_fn(self, rec_idx, est_notes):
        ...
        est_freqs = MetricsInferenceWithVoicingDetection.est_notes_to_hz_fn(est_notes)
...
class MetricsInferenceHardThreshold:

    def __init__(self, model):

        self.model = model
        self.num_recs = len(model.config.tvt_split_dict[model.name])
        self.th_logits = -0.7537717819213867
    ...
    @tf.function(input_signature=[
        ...
        tf.TensorSpec([None, 128], name='ref_notes'),
        tf.TensorSpec([None, 361, 128], name='logits'),
        tf.TensorSpec([], name='loss'),
    ...
    def _update_states_tf_fn(self, rec_idx, ref_notes, logits, loss, padded_frames):
        ...
        logits.set_shape([None, n_bins + 1, snippet_len])
        ...
        logit_peak_indices = tf.argmax(logits, axis=1, output_type=tf.int32)
        peak_logits = tf.gather(logits, axis=1, indices=logit_peak_indices, batch_dims=1)
        vth = tf.convert_to_tensor(self.th_logits, tf.float32)
        est_voicing = peak_logits > vth
        n_est_voicing = tf.logical_not(est_voicing)
        est_notes = MetricsDetermineThreshold.est_notes_360_fn(logit_peak_indices)
    ...
    def update_states(self, rec_idx_tf, batch_idx_tf, padded_frames_tf, ref_notes_tf, pitch_logits_torch, loss_torch):
        ...
        est_notes_with_voicing_info = self._update_states_tf_fn(
            rec_idx=rec_idx_tf,
            ref_notes=ref_notes_tf,
            pred_indices=pred_indices,
            loss=loss_torch.item(),
        ...
        self.est_notes.append(est_notes_with_voicing_info)
    ...
    def mir_eval_oa_fn(self, rec_idx, est_notes):
        ...
        est_freqs = MetricsInferenceHardThreshold.est_notes_to_hz_fn(est_notes)
...
class MetricsDetermineThreshold:
    ...
    @staticmethod
    def est_notes_360_fn(peak_indices):
    ...
    def update_states_tf_fn(self, rec_idx, ref_notes, logits, loss, padded_frames):
        ...
        est_notes = MetricsDetermineThreshold.est_notes_360_fn(est_peak_indices)
        est_ref_note_diffs = tf.abs(est_notes - ref_notes)
        est_ref_note_diffs.set_shape([None])
    ...
    def update_states_torch_tf(self, rec_idx_tf, batch_idx_tf, padded_frames_tf, ref_notes_tf, pitch_logits_torch, loss_torch):
        ...
        if DEBUG:
            ...
            ssf(pitch_logits_torch, [None, 361, 128])
        rec_idx_tf, batch_idx_tf, padded_frames_tf, ref_notes_tf = tf.identity_n([rec_idx_tf, batch_idx_tf, padded_frames_tf, ref_notes_tf])
        pitch_logits_torch = torch.clone(pitch_logits_torch)
        loss_torch = torch.clone(loss_torch)

        pitch_logits_tf = pitch_logits_torch.cpu().numpy()
        loss_tf = loss_torch.cpu().numpy()
...
class Model:
    ...
    def __init__(self, config, name):
        ...
        self.metrics = MetricsInferenceWithVoicingDetection(self)
...
def main():
    ...
    def validation_and_inference_fn(model_name, global_step=None):
        ...
        validation_in_inference = is_inferencing and model.name == 'validation'

        if validation_in_inference:
            th_metrics = model.threshold_metrics
            th_metrics.reset()
        ...
        with torch.no_grad():
            for batch_idx in range(batches_per_epoch):
                ...
                if validation_in_inference:
                    th_metrics.update_states_torch_tf(
                        rec_idx_tf=batch['rec_idx'],
                        batch_idx_tf=batch['batch_idx'],
                        padded_frames_tf=batch['padded_frames'],
                        ref_notes_tf=ref_notes_tf,
        ...
        if validation_in_inference:
            ...
            oa = metrics.oa
            th_logits = metrics.current_voicing_threshold
            t = np.exp(th_logits)
            th_prob = t / (1. + t)

            logging.info('\nvalidation - oa - {}'.format(oa))
            logging.info('th - logits - {} - prob - {}'.format(th_logits, th_prob))
