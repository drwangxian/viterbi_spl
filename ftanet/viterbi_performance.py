...
mir_eval
threshold prob = 0.37
training/validation/test    0.8032/0.7730/0.7476

comparison of different viterbi methods (mir_eval)
shaun:    0.8005/0.7721/0.7484
scaled:   0.5921/0.5674/0.5269
unscaled: 0.7642/0.7418/0.7213
...
class AcousticModel:
    ...
    def __init__(self, config):
        ...
        hard_th_prob = 0.37
        logits = np.log(hard_th_prob/ (1. - hard_th_prob))
        self.voicing_threshold_logit = logits

        viterbi_method = config.viterbi_method
        if viterbi_method == 'shaun':
            viterbi_ins = Viterbi(voicing_threshold_logit=self.voicing_threshold_logit)
        elif viterbi_method == 'softmax-scaled':
            viterbi_ins = SoftMaxViterbi(scaled=True)
        elif viterbi_method == 'softmax-unscaled':
            viterbi_ins = SoftMaxViterbi(scaled=False)
        else:
            assert False
        self.viterbi = viterbi_ins
...
class MetricsInference(MetricsBase):
    ...
    def __init__(self, model):
        ...
        self.voicing_threshold = model.config.acoustic_model_ins.voicing_threshold_logit
    ...
    def viterbi_update_states_tf_fn(self, rec_idx, ref_notes, melody_bins, est_voicing):
        ...
        est_notes = self.est_notes_320_fn(melody_bins)
...
class Viterbi:
    ...
    def __init__(self, voicing_threshold_logit):

        self.num_freq_bins = 320
        self.single_side_peak_width = 5

        self.threshold_logit = voicing_threshold_logit
        self.transition_matrix = self.transition_matrix_fn()

        self.ini_probs = self.init_probs_fn()
    ...
    def observation_probs_fn(self, logits):
        ...
        threshold = self.threshold_logit  # logit threshold
    ...
    def init_probs_fn(self):
        ...
        assert probs.argmax() == self.num_freq_bins
