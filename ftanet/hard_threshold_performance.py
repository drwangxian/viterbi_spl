...
mir_eval
threshold prob = 0.37
training/validation/test    0.8031/0.7729/0.7474
...
class MetricsInference(MetricsBase):
    ...
    def update_states_tf_fn(self, rec_idx, ref_notes, logits, loss):
        ...
        v_th_logits = self.model.config.acoustic_model_ins.voicing_threshold_logit
        v_th_logits = tf.convert_to_tensor(v_th_logits, tf.float32)
        est_voicing = est_peak_logits > v_th_logits
        n_est_voicing = tf.logical_not(est_voicing)
        ...
        est_notes = tf.where(est_voicing, est_notes, 0)
