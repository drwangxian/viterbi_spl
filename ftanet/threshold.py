...
adc04: 0.6730254245331757
mirex05: 0.7270342422776732
mdb: 0.7226050687333331
mir1k: 0.5925131542349042
rwc: 0.7066967685014418


22-May-2023 Mon
label by m2m3 method
mir_eval
training/validation/test    0.8023/0.7739/0.7517


24-May-2023 Wed
threshold:
[INFO] best threshold - prob - 0.37000000976324066 - logits - -0.5322167873382568
[INFO] hard threshold - oa - 0.7738596200942993 (tf, not mir_eval)
...
import acoustic_model as acoustic_model_module
import soundfile
import mir_eval
...
class Config:
    ...
    def __init__(self):
        ...
        self.train_or_inference = Namespace(
            inference='/media/hd/2023/05/22_shuai/ckpts/d0-51',
            from_ckpt=None,
            ckpt_prefix=None
        )
        ...
        self.snippet_len = N_TIME_FRAMES
        ...
        if self.train_or_inference.inference is None:
            ...
            spec = tf.random.normal([self.batch_size, self.snippet_len, N_FREQ_BINS, 3])
...
class AcousticModel:
    ...
    @tf.function(input_signature=[
        tf.TensorSpec([None, N_TIME_FRAMES], name='ref_notes'),
        tf.TensorSpec([None, N_TIME_FRAMES, N_FREQ_BINS + 1], name='logits')
    ], autograph=False)
    def loss_tf_fn(self, ref_notes, logits):
        # smoothing
        n_f_bins = N_FREQ_BINS
        n_t_frames = N_TIME_FRAMES

        note_range = TFDataset.note_range
        assert len(note_range) == n_f_bins and note_range[0] > 0
...
class TFDataset:
    ...
    def gen_label_fn(self, track_id):
        ...
        assert m2_melody.ndim == 2 and m2_melody.shape[1] == 2
    ...
    def note_out_of_range_chk_fn(self, np_dataset):
        ...
        model = self.model
        note_range = TFDataset.note_range
        assert len(note_range) == N_FREQ_BINS and note_range[0] > 0
...
class TFDatasetForTrainingModeTrainingSplit(TFDataset):
    ...
    def __init__(self, model):
        ...
        logging.info('training - generating spectrograms and labels')
    ...
    def map_idx_to_data_fn(self, idx):
        ...
        def py_fn(idx):
            ...
            if effective_num_frames < required_num_frames:
                ...
                if padded_frames:
                    spec = np.pad(spec, [[0, padded_frames], [0, 0], [0, 0]])
            ...
            spec = np.reshape(spec, [-1, snippet_len, n_freq_bins, 3])
        ...
        spec.set_shape([None, snippet_len, n_freq_bins, 3])
...
class MetricsBase:
    ...
    def est_notes_fn(logits):
        ...
        logits.set_shape([None, n_freq_bins + 1])
        note_range = TFDataset.note_range
        ...
        t.set_shape([None, n_freq_bins + 1])
...
class MetricsInference(MetricsBase):
    ...
    @tf.function(input_signature=[
        ...
        tf.TensorSpec([None, N_FREQ_BINS + 1], name='logits'),
        tf.TensorSpec([], name='loss')
    ], autograph=False)
    ...
    def mir_eval_oa_fn(self, rec_idx, est_notes):
        ...
        est_times = np.arange(num_frames) * 0.010
...
class MetricsDetermineThreshold:
    ...
    def __init__(self, model):
        ...
        p = np.arange(1, 100) * 0.01
        pb = 1. - p
        logits = np.log(p / pb)
        self.voicing_thresholds = logits.astype(np.float32)
    ...
    def est_notes_320_fn(self, peak_indices):

        note_range = TFDataset.note_range
        assert len(note_range) == self.n_bins
        assert note_range[0] > 0
        note_range = tf.convert_to_tensor(note_range, tf.float32)

        est_notes = tf.gather(note_range, indices=peak_indices)
    ...
    @tf.function(input_signature=[
        ...
        tf.TensorSpec([None, N_TIME_FRAMES, N_FREQ_BINS + 1], name='logits'),
        tf.TensorSpec([], name='loss'),
        tf.TensorSpec([], tf.int32, name='padded_frames')
    ], autograph=False)
    def _update_states_tf_fn(self, rec_idx, ref_notes, logits, loss, padded_frames):
        ...
        rec_idx.set_shape([])
        ref_notes = tf.convert_to_tensor(ref_notes, tf.float32)
        ref_notes.set_shape([None, snippet_len])

        logits = tf.convert_to_tensor(logits, tf.float32)
        logits.set_shape([None, snippet_len, n_bins + 1])
        loss = tf.convert_to_tensor(loss, tf.float32)
        ...
        logits = tf.reshape(logits, [-1, n_bins + 1])
        logits = tf.cond(padded, lambda: logits[:-padded_frames], lambda: logits)
        logits = logits[:, 1:] - logits[:, :1]
        logits.set_shape([None, n_bins])

        ref_voicing = ref_notes > .1
        n_ref_voicing = tf.logical_not(ref_voicing)

        est_peak_indices = tf.argmax(logits, axis=1, output_type=tf.int32)
        est_peak_logits = tf.gather(logits, indices=est_peak_indices, axis=1, batch_dims=1)
        est_voicing = est_peak_logits[:, None] > voicing_thresholds
        est_voicing.set_shape([None, num_voicing_thresholds])
        n_est_voicing = tf.logical_not(est_voicing)

        est_notes = self.est_notes_320_fn(est_peak_indices)
        est_ref_note_diffs = tf.abs(est_notes - ref_notes)
        est_ref_note_diffs.set_shape([None])
        ...
        voiced_frames = count_nonzero_fn(ref_voicing)
        unvoiced_frames = tf.size(ref_voicing, tf.int64) - voiced_frames
        ref_voicing_logical_and_est_voicing = ref_voicing[:, None] & est_voicing
        ...
        incorrect_voiced_frames = n_ref_voicing[:, None] & est_voicing
        incorrect_voiced_frames = count_nonzero_for_multi_ths_fn(incorrect_voiced_frames)
        correct_unvoiced_frames = n_ref_voicing[:, None] & n_est_voicing
        ...
        correct_pitches_wide = ref_voicing & correct_pitches_wide
        correct_pitches_wide.set_shape([None])
        correct_pitches_strict = correct_pitches_wide[:, None] & est_voicing
        ...
        correct_chromas_wide = ref_voicing & correct_chromas_wide
        correct_chromas_wide.set_shape([None])
        correct_chromas_strict = est_voicing & correct_chromas_wide[:, None]
    ...
    def best_voicing_threshold_fn(self):
        ...
        best_th_logits = self.voicing_thresholds[best_th_idx]
        t = np.exp(best_th_logits)
        t = t / (1. + t)
        self.current_voicing_threshold = t
        logging.info('best threshold - prob - {} - logits - {}'.format(t, best_th_logits))
...
class Model:
    ...
    def __init__(self, config, name):
        ...
        if self.name == 'validation':
            self.threshold_metrics = MetricsDetermineThreshold(self)
...
def main():
    ...
    def training_fn(global_step=None):
        ...
        for batch_idx in range(batches_per_epoch):
            ...
            with tf.GradientTape() as tape:
                ...
                loss = loss_fn(ref_notes=batch['notes'], logits=logits)
            grads = tape.gradient(loss, trainable_vars)
            optimizer.apply_gradients(zip(grads, trainable_vars))
            metrics.update_states(ref_notes=batch['notes'], logits=logits, loss=loss)
    ...
    def inference_fn(model_name, global_step=None):
        ...
        is_validation = model_name == 'validation'

        if is_validation:
            threshold_metrics = model.threshold_metrics
            threshold_metrics.reset()
        ...
        for batch_idx in range(batches_per_epoch):
            ...
            if is_validation:
                threshold_metrics.update_states(rec_idx=batch['rec_idx'],
                                                snippet_idx=batch['snippet_idx'],
                                                padded_frames=batch['padded_frames'],
                                                ref_notes=batch['notes'],
                                                logits=logits,
        ...
        if is_validation:
            _ = threshold_metrics.results()
            oa = threshold_metrics.oa
            logging.info('hard threshold - oa - {}'.format(oa))
