...
class Config:
    ...
    def __init__(self):
        ...
        self.tb_dir = 'tb_{}_inf_mdb'.format(self.method)
...
class TFDatasetForMirex05(TFDataset):
    ...
    def gen_np_dataset_fn(self):
        ...
        logging.info('mirex05 - {} - generating labels'.format(model.name))
        ...
        for track_idx, track_id in enumerate(track_ids):
            logging.debug('{}/{} - {}'.format(track_idx + 1, num_tracks, track_id))
            wav_file = os.path.join(os.environ['mirex05'], track_id + '.wav')
            self.wav_files.append(wav_file)
            labels = TFDatasetForMirex05.gen_label_fn(track_id)
            dataset.append(labels)
...
class TFDatasetForMir1k(TFDataset):
    ...
    def gen_np_dataset_fn(self):
        ...
        logging.info('mir1k - {} - generating labels'.format(model.name))
        ...
        for track_idx, track_id in enumerate(track_ids):
            ...
            wav_file = os.path.join(os.environ['mir1k'], 'Wavfile', track_id + '.wav')
            self.wav_files.append(wav_file)
            labels = TFDatasetForMir1k.gen_label_fn(track_id)
            dataset.append(labels)
...
class MetricsOriginal(MetricsBase):
    ...
    def update_states_fn(self, rec_idx, voicing, bins):

        num_recs = self.num_recs
        model = self.model

        assert isinstance(voicing, np.ndarray)
        assert voicing.dtype == np.bool_
        assert isinstance(bins, np.ndarray)
        assert bins.dtype in (np.int32, np.int64)

        assert rec_idx < num_recs
        ref_dict = model.dataset.np_dataset[rec_idx]
        ref_notes = ref_dict['notes']

        n_frames = len(ref_dict['notes'])
        _n_frames = len(voicing)
        t = n_frames - _n_frames
        if np.abs(t) > 1:
            logging.warning('large difference - {} frames'.format(t))

        if t > 0:
            voicing = np.pad(voicing, [[0, t]])
            bins = np.pad(bins, [[0, t]])
        elif t < 0:
            ref_notes = np.pad(ref_notes, [[0, -t]])
        assert len(ref_notes) == len(voicing) == len(bins)
        ...
        oa = self.mir_eval_oa_fn(rec_idx=rec_idx, est_notes=est_notes_with_voicing_info)
    ...
    def inference_fn(model_name, global_step=None):
        ...
        if method == 'original':
            for rec_idx, wav_file in enumerate(wav_files):
                t = AcousticModel.melody_fn(wav_file)
                voicing = t['voicing']
                bins = t['bins']
                metrics.update_states_fn(rec_idx=rec_idx, voicing=voicing, bins=bins)
        elif method == 'thresholding':
            for rec_idx, wav_file in enumerate(wav_files):
